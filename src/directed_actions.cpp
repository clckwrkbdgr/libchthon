...
void Move::commit(Monster & someone, Game & game)
{
    ...
    Monster & monster = find_at(game.current_level().monsters, new_pos);
	if(monster.valid()) {
...
void Fire::commit(Monster & someone, Game & game)
{
	...
	while(true) {
		...
		Object & object = find_at(game.current_level().objects, item.pos + shift);
		if(object.valid()) {
		...
		Monster & monster = find_at(game.current_level().monsters, item.pos + shift);
		if(monster.valid()) {
			item.pos += shift;
			game.current_level().items.push_back(item);
